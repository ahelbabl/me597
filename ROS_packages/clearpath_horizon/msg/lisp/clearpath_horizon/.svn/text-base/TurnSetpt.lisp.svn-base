...
(defclass <TurnSetpt> (ros-message)
  ((header
    :reader header-val
    :initarg :header
...
(defmethod serialize ((msg <TurnSetpt>) ostream)
  ...
  (let ((bits (roslisp-utils:encode-double-float-bits (slot-value msg 'turn_radius))))
    (write-byte (ldb (byte 8 0) bits) ostream)
    (write-byte (ldb (byte 8 8) bits) ostream)
    (write-byte (ldb (byte 8 16) bits) ostream)
    (write-byte (ldb (byte 8 24) bits) ostream)
...
(defmethod deserialize ((msg <TurnSetpt>) istream)
  ...
  (let ((bits 0))
    ...
    (setf (ldb (byte 8 48) bits) (read-byte istream))
    (setf (ldb (byte 8 56) bits) (read-byte istream))
    (setf (slot-value msg 'turn_radius) (roslisp-utils:decode-double-float-bits bits)))
  (let ((bits 0))
    (setf (ldb (byte 8 0) bits) (read-byte istream))
    (setf (ldb (byte 8 8) bits) (read-byte istream))
    (setf (ldb (byte 8 16) bits) (read-byte istream))
...
(defmethod ros-datatype ((msg (eql '<TurnSetpt>)))
  "Returns string type for a message object of type '<TurnSetpt>"
  "clearpath_horizon/TurnSetpt")
(defmethod md5sum ((type (eql '<TurnSetpt>)))
  "Returns md5sum for a message object of type '<TurnSetpt>"
  "023314e739de17bd5207788d54c661df")
(defmethod message-definition ((type (eql '<TurnSetpt>)))
  "Returns full string definition for message of type '<TurnSetpt>"
  (format nil "Header header~%float64 trans_vel~%float64 turn_radius~%float64 trans_accel~%~%================================================================================~%MSG: roslib/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.secs: seconds (stamp_secs) since epoch~%# * stamp.nsecs: nanoseconds since stamp_secs~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%# 0: no frame~%# 1: global frame~%string frame_id~%~%~%"))
(defmethod serialization-length ((msg <TurnSetpt>))
  (+ 0
...
(defmethod ros-message-to-list ((msg <TurnSetpt>))
  "Converts a ROS message object to a list"
  (list '<TurnSetpt>
    (cons ':header (header-val msg))
    (cons ':trans_vel (trans_vel-val msg))
    (cons ':turn_radius (turn_radius-val msg))
