...
void AckermannPlugin::LoadChild(XMLConfigNode *node)
{
  ...
  ros::SubscribeOptions::create<geometry_msgs::Twist>("/clearpath/robots/default/cmd_vel", 1,
               boost::bind(&AckermannPlugin::carDriveCallback, this, _1),
               ros::VoidPtr(), &queue_);

  sub_ = rosnode_->subscribe(so);
  pub_ = rosnode_->advertise<nav_msgs::Odometry>("odom", 1);
  pub2_ = rosnode_->advertise<clearpath_horizon::RawEncoders>("/clearpath/robots/default/data/raw_encoders", 1);
}
...
void AckermannPlugin::UpdateChild()
{
  ...
  odomVel[0] = dr * cos(odomPose[2])/ stepTime.Double();
  odomVel[1] = dr * sin(odomPose[2])/ stepTime.Double();
  odomVel[2] = da / stepTime.Double();

  if (enableMotors)
  {
    //joints[LEFT]->SetVelocity(0, wheelSpeed[LEFT] / (wd / 2.0));

    //joints[RIGHT]->SetVelocity(0, wheelSpeed[RIGHT] / (wd / 2.0));
    joints[LEFT]->SetForce(0, wheelTorque[LEFT]);
    joints[RIGHT]->SetForce(0, wheelTorque[RIGHT]);

    // FIXME: come up with something nicer for doing position control
    // than this simple proportional controller
    joints[STEER]->SetForce(0, P*(steerAngle - a) + 
                               D*(steerAngle - a - prevErr)/stepTime.Double() +
                               I*errSum);

    errSum += (prevErr + steerAngle - a)/2*stepTime.Double();
    prevErr = steerAngle - a;
...
void AckermannPlugin::GetPositionCmd()
{
  ...
  // steering angle
  wheelTorque[LEFT] = vr/* + a * **(wheelSepWidthP) / 2*/;
  wheelTorque[RIGHT] = vr/* - a * **(wheelSepWidthP) / 2*/;
  steerAngle = a;
...
void AckermannPlugin::carDriveCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
{
  ...
  torque_ = cmd_msg->linear.x/100.0*MAX_TORQUE;  
  angle_ = cmd_msg->angular.z/100.0*MAX_ANGLE;
...
void AckermannPlugin::publish_odometry()
{
  ...
  enc_.ticks[0] = (**(wheelDiamP))/2*joints[LEFT]->GetVelocity(0)/M_PER_TICK;
  pub2_.publish(enc_);
}
...
void AckermannPlugin::write_position_data()
{
  ...
  pos_iface_->data->pose.yaw = odomPose[2];
